...
│   ├── lib/*.js          vendored JS, appended to that same bundle verbatim
...
- `webfrontend.coffee1` files compile (CoffeeScript 1.x — the version the
  ...
  `webfrontend.js` files join that same bundle verbatim, after the compiled
  CoffeeScript and in list order: fylr loads only the one bundle named by
  `plugin.webfrontend.url`, so a vendored library that is not CoffeeScript
  (a viewer, a charting lib) has to be part of it — delivered next to the
  bundle it would never be loaded.
...
webfrontend:
  ...
  js:                    # plain JS (a vendored library): appended to that
    - webfrontend/lib/vendor.js         # same bundle verbatim, in this order
  scss:                  # compiled to same-named .css
